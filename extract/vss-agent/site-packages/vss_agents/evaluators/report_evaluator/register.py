...
async def register_report_evaluator(config: ReportEvaluatorConfig, builder: EvalBuilder):
    ...
    for method_name in unique_methods:
        ...
        if method_name not in METRIC_REGISTRY:
            ...
            raise ValueError(
                f"Unknown metric '{method_name}' found in config. " f"Available metrics: {available_metrics}"
            )
