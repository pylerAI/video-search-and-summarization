...
class ExtendedEvalOutputItem(EvalOutputItem):
    ...
    vlm_field_score: Optional[float] = Field(None, description="VLM field score for this report")
...
class ExtendedEvalOutput(EvalOutput):
    ...
    average_vlm_field_score: Optional[float] = None
...
class ReportEvaluatorConfig(EvaluatorBaseConfig, name="report_evaluator"):
    ...
    vlm_related_fields: Optional[List[str]] = Field(
        default=None,
...
async def _fetch_and_parse_report(
    object_store_client, response: str, url_pattern: str, camera_id: Optional[str] = None
...
class ReportEvaluator(BaseEvaluator):
    ...
    def __init__(
        ...
        vlm_related_fields: Optional[List[str]] = None,
    ...
    async def evaluate(self, eval_input_items: List[EvalInputItem]) -> ExtendedEvalOutput:
        ...
        if self.include_vlm_output:
            ...
            logger.info(
                f"Evaluation complete: average_score={avg_score_str}, " f"average_vlm_field_score={vlm_score_str}"
            )
        else:
            avg_score_str = f"{result.average_score:.4f}" if result.average_score is not None else "N/A"
            logger.info(f"Evaluation complete: average_score={avg_score_str} " f"(VLM field score disabled)")
    ...
    async def evaluate_item(self, item: EvalInputItem) -> ExtendedEvalOutputItem:
        ...
        try:
            ...
            # Load reference
            reference_path = Path(answer) if Path(answer).is_absolute() else Path.cwd() / answer
            ...
            if self.include_vlm_output and self.vlm_related_fields:
                ...
                for section_name in self.vlm_related_fields:
                    if section_name in result.field_scores and result.field_scores[section_name] is not None:
                        section_score = result.field_scores[section_name].section_score
        ...
        if is_section:
            ...
            if allow_dynamic_discovery:
                ...
                if actual_unspecified:
                    llm_judge = self.metric_instances["llm_judge"]
        ...
        try:
            if method == "average":
                if not field_scores:
                    ...
                    scores = [fs.section_score or 0.0 for fs in field_scores.values()]
    ...
    async def _score_value(self, reference: Any, actual: Any, method: str, path: List[str]) -> Optional[float]:
