...
class VstHandler:
    def __init__(self, config: dict):
        ...
        self._cache_duration = self.config.get("vst_config", {}).get(
            "stream_status_cache_duration", 60
        )  # Default 60 seconds
        self.add_overlay = self.config.get("vst_config", {}).get("add_overlay", False)
        self.base_url = self.config.get("vst_config", {}).get("internal_url")
        if not self.base_url:
            raise ValueError("VST base URL not configured")

    def _compute_effective_time_window(self, start_time: str, end_time: str) -> Tuple[str, str]:
        """
        ...
        Behavior overview:
        - Inputs must be ISO-8601 UTC strings; missing/invalid values raise
          ValueError.
        ...
        anchor, duration = self._determine_anchor_and_duration()

        start_dt = self._parse_required_iso_utc(start_time, "start_time")
        end_dt = self._parse_required_iso_utc(end_time, "end_time")

        if duration <= 0:
            eff_start_str, eff_end_str = self._pass_through_with_minimum(start_time, end_time, start_dt, end_dt)
        ...
        eff_start, eff_end = self._window_start_anchor(start_dt, duration)
        eff_start_str = start_time
        eff_end_str = self._normalize_to_z(eff_end)
        self._log_effective_window(anchor, start_time, end_time, eff_start_str, eff_end_str)
        return eff_start_str, eff_end_str
    ...
    def _determine_anchor_and_duration(self) -> Tuple[str, int]:
        """Read vst_config and return (anchor, duration_in_seconds)."""
        vst_cfg = self.config.get("vst_config", {}) or {}
        anchor = (vst_cfg.get("segment_anchor") or "start").lower()
    ...
    @staticmethod
    def _window_start_anchor(start_dt: datetime, duration: int) -> Tuple[datetime, datetime]:
        """Return [start, start + duration] for start-anchored mode."""
        return start_dt, start_dt + timedelta(seconds=duration)

    def _window_end_anchor(
        self, start_dt: datetime, end_dt: datetime, duration: int, now_dt: datetime
    ...
    def _get_storage_config(self) -> Tuple[str, str, int]:
        ...
        storage_base = storage_cfg.get("internal_url") or self.base_url
    ...
    def _extract_media_path(self, payload: dict) -> Optional[str]:
        ...
        return payload.get("mediaFilePath") or payload.get("media_file_path") or payload.get("mediafilepath")

    def _resolve_media_path(self, vst_id: str, media_path: str) -> str:
        """Apply optional base-dir mapping and log final resolved path."""
        base_dir = os.environ.get("ALERT_REVIEW_MEDIA_BASE_DIR") or self.config.get("ALERT_REVIEW_MEDIA_BASE_DIR")
        if base_dir:
        ...
        self.logger.debug(f"VST media path resolved without base dir: vst_id={vst_id}, path={media_path}")
        return media_path
    ...
    def get_media_file_path_by_vst_id(self, vst_id: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.debug(f"VST storage lookup: connecting url={url}, vst_id={vst_id}, timeout={timeout}s")

            payload = self._request_storage_lookup(url, vst_id, timeout)
            self.logger.debug(f"VST storage lookup: response received for vst_id={vst_id}")
            media_path = self._extract_media_path(payload)
            if not media_path:
                self.logger.error(f"Storage lookup for vst_id={vst_id} did not include mediaFilePath")
                return None

            return self._resolve_media_path(vst_id, media_path)

        except requests.RequestException as e:
            self.logger.error(f"VST storage lookup failed for vst_id={vst_id}: {e}", exc_info=True)
            return None
    ...
    def build_sensor_id_sensor_name_mapping(self, sensor_data):
        ...
        for sensor_dict in sensor_data:
            for _sensor_id, sensor_info_list in sensor_dict.items():
            ...
            for _sensor_id, sensor_info_list in sensor_dict.items():
                for sensor_info in sensor_info_list:
                    if sensor_info.get("vodUrl"):
                        logger.debug(f"Processing sensor data item: {sensor_info}")
                        sensor_name_rtsp_mapping[sensor_info["name"]] = sensor_info["vodUrl"]
                    else:
                        logger.debug(f"Skipping sensor data item with missing 'vodUrl': {sensor_info}")

        logger.debug(f"Built sensor name to RTSP URL mapping: {sensor_name_rtsp_mapping}")
    ...
    def build_vst_urls(self, es_results, sensor_dict, base_url, video_buffer_time=10):
        ...
        for entry in es_results:
            ...
            start_time = datetime.strptime(entry["start"], "%Y-%m-%dT%H:%M:%S.%fZ") - timedelta(
                seconds=video_buffer_time
            )
            end_time = datetime.strptime(entry["end"], "%Y-%m-%dT%H:%M:%S.%fZ") + timedelta(seconds=video_buffer_time)
    ...
    def get_video_stream_url(
        ...
        Args:
            stream_id: The stream identifier (e.g., 'Building_K_Cam1_clip1')
            ...
            expiry_minutes: The expiry minutes for the video stream (default: 2)

        Returns:
            API response schema for reference
            {'absolutePath': '/home/vst/vst_release/webroot/temp_videos/Endeavor_Cafeteria_2025-09-10_11_16:31_928__2025-09-10_11_16:33_918_.mp4', 'baseUrl': 'http://10.63.141.62:30011', 'endTime': '2025-09-10T11:16:33.918Z', 'expiryISO': '2025-09-10T11:19:23.675Z', 'expiryMinutes': 2, 'expiryTimestamp': 1757503163675, 'fileName': 'Endeavor_Cafeteria_2025-09-10_11_16:31_928__2025-09-10_11_16:33_918_.mp4', 'startTime': '2025-09-10T11:16:31.928Z', 'streamId': 'Endeavor_Cafeteria', 'videoUrl': 'http://10.63.141.62:30011/temp_videos/Endeavor_Cafeteria_2025-09-10_11_16:31_928__2025-09-10_11_16:33_918_.mp4'}
        ...
        try:
            ...
            effective_start_time, effective_end_time = self._compute_effective_time_window(start_time, end_time)

            # if not self.check_time_in_recording_with_retries(stream_id, effective_start_time, effective_end_time, latency):
            #     return None
            ...
            if self.add_overlay and not remove_overlay:
                configuration["overlay"] = {
                    "bbox": {"showAll": False, "objectId": objects_ids},
                    "color": "green",
                    "thickness": 5,
                    "debug": True,
                    "opacity": 254,
            ...
            if not video_url:
                raise KeyError("videoUrl not found in response")

            return video_url

        except requests.RequestException as e:
            self.logger.error(f"Failed to get VST video URL: {e}")
            raise
        except Exception as e:
            self.logger.error(f"Unexpected error in get_video_stream_url: {e}")
            raise
    ...
    def download_clips(self, results_download):
        ...
        delay_time = self.config.get("vst_config", {}).get("delay_before_processing", 0)
        ...
        for entry in results_download:
            ...
            def attempt_download(
                download_url: str = url, download_file_path: str = file_path, download_entry: dict = entry
            ):
                try:
                    ...
                    with open(download_file_path, "wb") as file:
                        for chunk in response.iter_content(chunk_size=8192):
                            file.write(chunk)
                    logger.info(f"Downloaded: {download_file_path}")
                    download_entry["download_status"] = "Complete"
                    download_entry["file_path"] = download_file_path
                    return True
                except requests.exceptions.RequestException as e:
                    logger.error(f"Failed to download {download_url}: {e}")
                    download_entry["download_status"] = "Not Complete"
                    return False

            success = attempt_download()

            if not success:
                logger.info(f"Retrying download for alert ID: {alert_id} after {delay_time} seconds.")
    ...
    def _download_image(self, timestamp: str, stream_id: str, output_file: str) -> bool:
        ...
        try:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.debug(
                            f"Error downloading image on attempt {attempt + 1}: HTTP status {response.status_code}"
                        )
                except Exception as e:
            ...
            logger.debug(f"Exception while downloading image: {e!s}")
            return False

    def get_vst_rtsp_urls(self):
        vst_base_url = self.config["vst_config"]["internal_url"]
    ...
    def check_time_in_recording(self, stream_id, start_time_str, end_time_str):
        ...
        try:
            ...
            headers = {"Accept": "application/json, text/plain, */*", "streamId": stream_id}
            ...
            logger.debug(
                f"Time range {start_dt} to {end_dt} not found in recording timelines (start={timelines[0]['startTime']}, end={timelines[0]['endTime']})"
            )
            return False
            ...
            for idx, row in result_df.iterrows():
                try:
                    ...
                    if result:
                        ...
                        self.logger.debug(
                            f"Successfully processed sensor {row['sensorName']} " f"(stream_id: {stream_id})"
                        )
                    else:
                        self.logger.warning(f"Could not get snapshot for sensor {row['sensorName']}")

                except Exception as e:
                    self.logger.error(f"Error processing sensor {row['sensorName']}: {e}", exc_info=True)

            # Filter out rows without required data
            processed_df = result_df.dropna(subset=["streamId", "imageUrl", "sampledImage"])

            self.logger.info(f"Processed {len(processed_df)} out of {len(entities_df)} entities successfully")
            return processed_df
