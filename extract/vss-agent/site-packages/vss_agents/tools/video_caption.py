...
class VideoCaptionInput(BaseModel):
    ...
    @classmethod
    def validate_end_timestamp(cls, info: dict):
...
async def call_vlm_partition(llm, base64_frames, template_prompt, user_prompt, start_timestamp, fps, max_retries):
...
async def video_caption(config: VideoCaptionConfig, builder: Builder):
