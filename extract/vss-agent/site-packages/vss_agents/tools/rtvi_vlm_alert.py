...
class RTVIVLMAlertConfig(FunctionBaseConfig, name="rtvi_vlm_alert"):
    ...
    va_get_incidents_tool: Optional[FunctionRef] = Field(
        default=None,
...
class RTVIVLMAlertOutput(BaseModel):
    ...
    sensor_name: str | None = Field(None, description="Sensor name")
    stream_id: str | None = Field(None, description="RTVI-VLM stream ID (UUID)")
    message: str = Field(..., description="Status message")
    incidents: list[dict] | None = Field(None, description="List of incidents (for get_incidents action)")
    total_count: int | None = Field(None, description="Total number of incidents found (for get_incidents action)")
...
async def rtvi_vlm_alert(config: RTVIVLMAlertConfig, builder: Builder) -> AsyncGenerator[FunctionInfo]:
    ...
    async def _get_live_streams() -> dict[str, dict]:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(vst_url) as response:
                ...
                if input_data.action == "start":
                    ...
                    if sensor_name not in live_streams:
                        return RTVIVLMAlertOutput(
                            ...
                            f"Available sensors: {sorted(list(live_streams.keys()))}",
                        )
