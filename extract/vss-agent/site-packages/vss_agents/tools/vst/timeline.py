...
async def get_timeline(stream_id: str, vst_internal_url: str | None = None) -> (str, str):
    """
            ...
            with retry:
                try:
                    async with session.get(timelines_url) as response:
                        ...
                        return timeline_list[0].get("startTime"), timeline_list[0].get("endTime")
...
async def vst_timeline(config: VSTTimelineConfig, _: Builder):
    async def _vst_timeline(vst_timeline_input: VSTTimelineInput) -> str:
