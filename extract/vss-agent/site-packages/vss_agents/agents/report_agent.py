    ...
    if va_mcp_enabled:
        ...
        async def _execute_report(
            ...
            try:
                ...
                execution_time_ms = int((time.time() - execution_start_time) * 1000)
                error_output = AgentOutput(
                    messages=[f"Report Agent: Error generating video analysis report: {e!s}"],
                    status="error",
                    error_message=f"Report Agent: Failed to generate video analysis report: {e!s}",
                    metadata={
                        "generation_time_ms": execution_time_ms,
