...
from .es_client import ESClient, BASE_QUERY_TEMPLATE
from .query_builders import (
    IncidentQueryBuilder,
    BehaviorQueryBuilder,
    FramesQueryBuilder
)
from .utils import (
    build_sensor_map,
    ...
    create_empty_histogram_buckets,
    create_events_from_incidents,
...
class GetSensorIdsInput(BaseModel):
    ...
    place: Optional[str] = Field(default=None, description="Optional place name to filter sensor IDs")


class GetIncidentInput(BaseModel):
    """Input for get_incident function."""
    id: str = Field(description="The incident ID to retrieve")
    includes: Optional[list[str]] = Field(default=None, description="The metadata fields to include in the output")


class GetIncidentsInputBase(BaseModel):
    """Base input for get_incidents function (without VLM verdict)."""
    source: Optional[str] = Field(default=None, description="Optional source of the incidents (sensor ID or place/city name). If provided, source_type must also be provided. Place can be exact name or natural language description of place.")
    start_time: Optional[str] = Field(default=None, description="Optional start time of query (ISO format: YYYY-MM-DDTHH:MM:SS.sssZ). If omitted, returns the most recent incidents up to max_count.")
    end_time: Optional[str] = Field(default=None, description="Optional end time of query (ISO format: YYYY-MM-DDTHH:MM:SS.sssZ). If omitted, returns the most recent incidents up to max_count.")
    source_type: Optional[str] = Field(default=None, description="The type of the source (must be 'sensor' or 'place'). 'place' uses wildcard matching and can match city names or intersection names. Required if source is provided.")
...
class GetIncidentsInputWithVLM(GetIncidentsInputBase):
    ...
    vlm_verdict: Optional[str] = Field(
        default=None, 
    ...
    @classmethod
    def validate_vlm_verdict(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            if v not in allowed_verdicts:
                raise ValueError(
                    f"Video Analytics: vlm_verdict must be one of {allowed_verdicts}, got: '{v}'"
                )
        return v
...
class FovHistogramInput(BaseModel):
    ...
    object_type: Optional[str] = Field(default=None, description="Optional type of the object to filter by")
...
class VideoAnalyticsToolConfig(FunctionGroupBaseConfig, name="video_analytics"):
    ...
    index_prefix: str = Field(
        default="",
        description="Index prefix for all ES indexes"
    )
    vlm_verified: bool = Field(
        default=False,
        description="If true, query VLM verified incidents index instead of regular incidents"
    )
    vst_sensor_list_tool: Optional[FunctionRef] = Field(
        default=None,
        description="Optional VST sensor list tool to filter active sensors"
    )
...
async def video_analytics(_config: VideoAnalyticsToolConfig, _builder: Builder) -> AsyncGenerator[FunctionGroup, None]:
    """
    ...
    try:
        calibration_result = await es_client.get_by_id(
            index_key="calibration",
            doc_id="calibration"
        )
        if calibration_result:
            ...
            if _config.embedding_model_name:
                try:
                    from .embeddings import EmbeddingModel, PlaceEmbeddingCache
    ...
    async def _get_vst_sensor_names() -> Optional[set[str]]:
        """
        ...
        try:
            ...
            return result
        except (json.JSONDecodeError, KeyError, ConnectionError, ValueError) as e:
            return None
        except Exception as e:
            return None
        ...
        if (
            ...
            if matched_places:
                ...
                query["query"]["bool"]["should"] = [
                    {"wildcard": {"place.name.keyword": f"*{place_name}*"}}
                    for place_name in matched_places
                ]
                ...
                if _config.vlm_verified and vlm_verdict is not None:
                    if vlm_verdict == "all":
                        ...
                        query["query"]["bool"]["must"].append(
                            {"term": {"info.verdict.keyword": vlm_verdict}}
                        )
    ...
    async def _get_places(input: EmptyInput) -> dict:
        """
    ...
    async def _get_fov_histogram(input: FovHistogramInput) -> dict:
        ...
        results = await es_client.aggregate(
            index_key="frames",
            query_body=query,
            aggs=aggs
        )
        ...
        if results and "eventsOverTime" in results:
            for time_bucket in results["eventsOverTime"].get("buckets", []):
                ...
                for obj_bucket in object_type_buckets:
                    ...
                    bucket_data["objects"].append({
                        "type": obj_type,
                        "averageCount": round(avg_count) if avg_count else 0
                    })
        ...
        for empty_bucket in empty_histogram:
            if empty_bucket["start"] in bucket_map:
                ...
                bucket = bucket_map[empty_bucket["start"]]
            else:
                # Use empty bucket
                bucket = empty_bucket
            
            # Ensure all object types are represented (with 0 if missing)
            existing_types = {obj["type"] for obj in bucket["objects"]}
            for obj_type in object_types:
                if obj_type not in existing_types:
                    bucket["objects"].append({"type": obj_type, "averageCount": 0})
            
            # Sort objects by type for consistency
            bucket["objects"].sort(key=lambda x: x["type"])
            histogram.append(bucket)
        
        return {
            "bucketSizeInSec": bucket_size_sec,
            "histogram": histogram
        }
    ...
    async def _analyze(input: AnalyzeInput) -> str:
        ...
        if input.analysis_type == "max_min_incidents":
            ...
            result = await _get_average_speeds(AverageSpeedsInput(
                source=input.source,
                start_time=input.start_time,
                end_time=input.end_time,
                source_type=input.source_type
            ...
            if metrics:
                ...
                return f"Average speeds at {input.source} between {input.start_time} and {input.end_time}: {speed_summary}"
            else:
            ...
            result = await _get_fov_histogram(FovHistogramInput(
                source=input.source,
                start_time=input.start_time,
                end_time=input.end_time,
                object_type="Person"
    ...
    if "get_incident" in _config.include:
        group.add_function(
            name="get_incident",
            fn=_get_incident,
            description=_get_incident.__doc__
        )
    ...
    if "get_incidents" in _config.include:
        ...
        if _config.vlm_verified:
            ...
            group.add_function(
                name="get_incidents",
                fn=_get_incidents_vlm,
                description=_get_incidents.__doc__
            )
        else:
    ...
    if "get_sensor_ids" in _config.include:
        group.add_function(
            name="get_sensor_ids",
            fn=_get_sensor_ids,
            description=_get_sensor_ids.__doc__
        )
    ...
    if "get_places" in _config.include:
        group.add_function(
            name="get_places",
            fn=_get_places,
            description=_get_places.__doc__
        )
    
    if "get_fov_histogram" in _config.include:
        group.add_function(
            name="get_fov_histogram",
            fn=_get_fov_histogram,
            description=_get_fov_histogram.__doc__
        )
    
    if "get_average_speeds" in _config.include:
        group.add_function(
            name="get_average_speeds",
            fn=_get_average_speeds,
            description=_get_average_speeds.__doc__
        )
