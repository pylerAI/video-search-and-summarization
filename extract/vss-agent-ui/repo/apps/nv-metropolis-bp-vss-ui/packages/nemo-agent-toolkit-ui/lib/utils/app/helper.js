...
export const compressImage = (base64, mimeType, shouldCompress, callback)=>{
...
export const getWorkflowName = ()=>{
    const workflow = getURLQueryParam({
        param: 'workflow'
    }) || env('NEXT_PUBLIC_WORKFLOW') || process?.env?.NEXT_PUBLIC_WORKFLOW || APPLICATION_NAME;
    return workflow;
};
...
export const generateContentIntermediate = (intermediateSteps = [])=>{
    const generateDetails = (data, isParentLast = true)=>{
        try {
 ...
 */ export const handleIncompleteAgentThinkStepTags = (str = '')=>{
    try {
        ...
        if (numIncomplete > 0 && openingTags.length > 0) {
            ...
            str = str.substring(0, lastTag.index) + modifiedTag + str.substring(lastTag.index + lastTag.fullTag.length);
            // Add temporary closing tags for incomplete steps
            const closingTagsToAdd = Array(numIncomplete).fill('</agent-think-step>').join('');
 ...
 */ export const handleIncompleteAgentThinkTags = (str = '')=>{
    try {
        ...
        if (numIncomplete > 0 && openingTags.length > 0) {
            ...
            const closingTagsToAdd = Array(numIncomplete).fill('</agent-think>').join('');
            str = str + closingTagsToAdd;
        }
        return str;
    } catch (error) {
 ...
 */ export const isSystemResponseMessage = (message)=>{
    return message?.type === 'system_response_message';
};
export const isSystemIntermediateMessage = (message)=>{
    return message?.type === 'system_intermediate_message';
};
export const isSystemInteractionMessage = (message)=>{
 ...
 */ export const validateWebSocketMessage = (message)=>{
    return message && typeof message === 'object' && message.type && message.conversation_id;
};
/**
 * Extracts OAuth URL from interaction message
 */ export const extractOAuthUrl = (message)=>{
 ...
 */ export const shouldAppendResponseContent = (message)=>{
    return message.status === 'in_progress' && !!message.content?.text?.trim();
};
/**
 * Creates a new assistant message from WebSocket data
 */ export const createAssistantMessage = (id, parentId, content, intermediateSteps = [], humanInteractionMessages = [], errorMessages = [])=>{
    return {
        role: 'assistant',
        id,
        parentId,
 ...
 */ export const appendToAssistantContent = (existingContent, incomingText)=>{
 ...
 */ export const shouldRenderAssistantMessage = (message)=>{
