...
import { InteractionModal } from "./ChatInteractionMessage";
import HomeContext from "../../pages/api/home/home.context";
import { validateWebSocketMessageWithConversationId, isSystemResponseMessage, isSystemIntermediateMessage, isSystemInteractionMessage, isErrorMessage, isSystemResponseComplete, extractOAuthUrl } from "../../types/websocket";
import { getEndpoint } from "../../utils/app/api";
import { webSocketMessageTypes } from "../../utils/app/const";
import { saveConversation, saveConversations, updateConversation } from "../../utils/app/conversation";
import { fetchLastMessage, processIntermediateMessage } from "../../utils/app/helper";
import { shouldAppendResponse, appendAssistantText, mergeIntermediateSteps, applyMessageUpdate, createAssistantMessage, updateAssistantMessage, shouldRenderAssistantMessage } from "../../utils/chatTransform";
...
function extractSsePayloads(buffer) {
    ...
    for (const block of parts){
        ...
        const dataLines = block.split('\n').filter((line)=>/^data:\s*/.test(line)).map((line)=>line.replace(/^data:\s*/, '').trim()).filter((line)=>line.length > 0);
...
function parsePossiblyConcatenatedJson(payload) {
    // Fast path
...
export const Chat = ()=>{
    ...
    const webSocketModeRef = useRef(webSocketMode);
    ...
    const activeUserMessageId = useRef(null);
    ...
    const wsLastDispatchTime = useRef(0);
    const wsPendingUpdate = useRef(null);
    const wsFlushTimeout = useRef(null);
    // Store custom agent params for use in handleSend
    const customAgentParamsRef = useRef(null);
   ...
   */ const handleStopConversation = useCallback(()=>{
    ...
    const handleUserInteraction = ({ interactionMessage = {}, userResponse = '' })=>{
    ...
    // NOTE: We manually update conversationsRef.current in updateRefsAndDispatch and handleSend
    // to avoid race conditions with WebSocket messages.
    ...
    const connectWebSocket = async (retryCount = 0)=>{
        ...
        return new Promise((resolve)=>{
            ...
            let wsUrl = sessionStorage.getItem('webSocketURL') || webSocketURL || 'ws://127.0.0.1:8000/websocket';
            ...
            websocketLoadingToastId = toast.loading('WebSocket is not connected, trying to connect...', {
   ...
   */ const handleOAuthConsent = (message)=>{
        ...
        if (message.content?.input_type === 'oauth_consent') {
            ...
            if (oauthUrl) {
                const popup = window.open(oauthUrl, 'oauth-popup', 'width=600,height=700,scrollbars=yes,resizable=yes');
   ...
   */ const updateRefsAndDispatch = (updatedConversations, updatedConversation, currentSelectedConversation)=>{
   ...
   */ const processSystemResponseMessage = (message, messages)=>{
        if (!shouldAppendResponse(message)) {
            return messages;
        }
        const incomingText = isSystemResponseMessage(message) ? message.content?.text || '' : '';
        ...
        if (isLastAssistant) {
            ...
            return messages.map((m, idx)=>idx === messages.length - 1 ? updateAssistantMessage(m, combinedContent) : m);
        } else {
   ...
   */ const processIntermediateStepMessage = (message, messages)=>{
        ...
        if (!isLastAssistant) {
            ...
            const mergedSteps = mergeIntermediateSteps(lastSteps, message, sessionStorage.getItem('intermediateStepOverride') === 'false' ? false : Boolean(intermediateStepOverride));
            return messages.map((m, idx)=>idx === lastIdx ? updateAssistantMessage(m, m.content, mergedSteps) : m);
        }
    };
    /**
   * Processes error messages by attaching them to assistant messages
   */ const processErrorMessage = (message, messages)=>{
        if (!isErrorMessage(message)) return messages;
        const lastMessage = messages.at(-1);
        const isLastAssistant = lastMessage?.role === 'assistant';
        if (isLastAssistant) {
            // Attach error to existing assistant message
            return messages.map((m, idx)=>idx === messages.length - 1 ? {
                    ...m,
                    errorMessages: [
                        ...m.errorMessages || [],
                        message
   ...
   */ const flushWsPendingUpdate = ()=>{
        ...
        const targetConversation = currentConversations.find((c)=>c.id === conversationId);
        if (!targetConversation) {
            wsPendingUpdate.current = null;
            return;
        }
        // Update conversation with pending messages
        const updatedConversation = applyMessageUpdate(targetConversation, messages);
        // Update conversations array
        const updatedConversations = currentConversations.map((c)=>c.id === updatedConversation.id ? updatedConversation : c);
        // Update state and persistence
        updateRefsAndDispatch(updatedConversations, updatedConversation, selectedConversationRef.current);
        wsPendingUpdate.current = null;
   ...
   */ const handleWebSocketMessage = (message)=>{
        ...
        try {
            ...
            console.error('Available conversations:', conversationsRef.current?.map((c)=>({
                    id: c.id,
                    name: c.name
                })));
        ...
        if (sessionStorage.getItem('enableIntermediateSteps') === 'false' && isSystemIntermediateMessage(message)) {
            return;
        }
        // Find target conversation with enhanced error reporting
        const currentConversations = conversationsRef.current;
        const targetConversation = currentConversations.find((c)=>c.id === message.conversation_id);
        if (!targetConversation) {
            return;
        }
        ...
        let baseMessages = pending && pending.conversationId === message.conversation_id ? pending.messages : targetConversation.messages;
    ...
    const handleSend = useCallback(async (message, deleteCount = 0, retry = false)=>{
        ...
        if (selectedConversation) {
            ...
            if (webSocketModeRef?.current) {
                ...
                const currentConversations = conversationsRef.current || [];
                const conversationExists = currentConversations.some((c)=>c.id === selectedConversation.id);
                ...
                let chatMessages;
                if (chatHistory) {
                    chatMessages = updatedConversation?.messages?.map((message)=>{
                        return {
                            role: message.role,
                            content: [
                                {
                                ...
                                ...typeof message?.content === 'object' && message?.content && 'attachments' in message.content && message.content.attachments?.length > 0 ? message.content.attachments?.map((attachment)=>({
                                        type: 'image',
            ...
            const chatBody = {
                messages: chatHistory ? messagesCleaned : [
                    {
                        role: 'user',
                ...
                additionalProps: {
                    enableIntermediateSteps: sessionStorage.getItem('enableIntermediateSteps') ? sessionStorage.getItem('enableIntermediateSteps') === 'true' : enableIntermediateSteps
                },
                // Spread custom params at top level
            ...
            try {
                ...
                if (!false) {
                    ...
                    let pendingIntermediateSteps = []; // Accumulate intermediate steps between dispatches
                    ...
                    while(!done){
                        ...
                        for (const stepMatch of stepMatches){
                            try {
                                ...
                                let rawIntermediateMessage = tryParseJson(jsonString);
    ...
    const handleEditMessage = useCallback((editedMessage, deleteCount)=>{
    ...
    const handleDeleteMessage = useCallback((messageIndex)=>{
        ...
        const { single, all } = updateConversation(updatedConversation, allConversations || []);
    ...
    ]); // Only re-run if the container ref changes
    ...
    const handleScroll = useCallback(()=>{
        ...
        // Only disable auto-scroll if it's a user-initiated upward scroll during streaming
        if (isUserInitiatedScroll.current && isScrollingUp && autoScrollEnabled && messageIsStreaming) {
            setAutoScrollEnabled(false);
            homeDispatch({
                field: 'autoScroll',
                value: false
    ...
    useEffect(()=>{
        throttledScrollDown();
        selectedConversation && setCurrentMessage(()=>{
    ...
    return /*#__PURE__*/ _jsx("div", {
        ...
        children: /*#__PURE__*/ _jsxs(_Fragment, {
            children: [
                /*#__PURE__*/ _jsxs("div", {
                    ...
                    children: [
                        ...
                        selectedConversation?.messages.map((message, index, arr)=>{
                            ...
                            return /*#__PURE__*/ _jsx(MemoizedChatMessage, {
                                message: message,
                                messageIndex: index,
                        ...
                        loading && /*#__PURE__*/ _jsx(ChatLoader, {
                            statusUpdateText: `Thinking...`
                        }),
                        /*#__PURE__*/ _jsx("div", {
                ...
                /*#__PURE__*/ _jsx(InteractionModal, {
                    isOpen: modalOpen,
