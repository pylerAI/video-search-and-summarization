...
class VlmRequestParams:
    """Request parameters for VLM processing, including dynamic model selection.
    
    This class carries both inference parameters (prompt, generation config) and
    model configuration (for dynamic model switching). Each request can specify
    a different model from the registry, enabling multi-tenant and adaptive scenarios.
    
    Attributes:
        vlm_generation_config: Generation parameters (temperature, max_tokens, etc.)
        vlm_prompt: User prompt for inference
        model_id: Model identifier from registry (e.g., 'gpt-4o', 'claude-3.5-sonnet')
        model_endpoint: API endpoint URL for external model
        model_api_key: Authentication key for model API
        model_deployment_name: Deployment-specific model name
        model_additional_headers: Optional HTTP headers for API calls
    """
    ...
    def has_model_config(self) -> bool:
        """Check if this request has complete model configuration.
        
        Used to determine whether dynamic model switching should be attempted.
        All three fields (model_id, model_endpoint, model_deployment_name) must
        be present for a valid configuration.
        
        Returns:
            bool: True if all required model config fields are present, False otherwise.
        """
...
class DecoderProcess(ViaProcessBase):
    ...
    def _initialize(self):
        ...
        if self._vlm_model_type == VlmModelType.OPENAI_COMPATIBLE:
            ...
            raise Exception(f"Unsupported VLM model type for decoder: {self._vlm_model_type}")
...
class EmbeddingProcess(ViaProcessBase):
    ...
    def _initialize(self):
        ...
        if self._vlm_model_type == VlmModelType.OPENAI_COMPATIBLE:
            ...
            raise Exception(f"Unsupported VLM model type for embedding: {self._vlm_model_type}")
...
class VlmProcess(ViaProcessBase):
    ...
    def _initialize(self):
        ...
        # Only OpenAI compatible model registry approach is supported
        if self._vlm_model_type == VlmModelType.OPENAI_COMPATIBLE:
            ...
            raise Exception(f"Unsupported VLM model type: {self._vlm_model_type}")
        return True
    ...
    def _can_batch(self, item1, item2):
        # Batching only supported if request params are identical
        return item1["request_params"] == item2["request_params"]

    def _is_busy(self):
        if self._model and hasattr(self._model, "can_enqueue_requests"):
    ...
    def _update_model_config_if_needed(self, request_params):
        """Dynamically switch VLM model based on request parameters.
        
        This method enables per-request model selection - each user request can specify
        a different model from the registry, and this method will reinitialize the
        CompOpenAIModel client only when the configuration actually changes.
        
        Smart Caching: Compares the new configuration to the current configuration and
        skips reinitialization if unchanged. This optimizes the common case where
        consecutive requests use the same model.
        
        Multiprocessing: This runs inside the worker process, which owns the model
        instance. Configuration flows through VlmRequestParams (serializable).
        
        Args:
            request_params (VlmRequestParams): Request containing model configuration.
        
        Raises:
            Exception: If model initialization fails (invalid config, connection error, etc.).
                      Failures are propagated to the client as error responses.
        
        Note:
            Only applicable for OpenAI-compatible models. Internal models ignore this.
        """
    ...
    def _process(
        ...
        if self._vlm_model_type == VlmModelType.OPENAI_COMPATIBLE:
            ...
            raise Exception(f"Unsupported VLM model type for context: {self._vlm_model_type}")

        embeds = []
        frame_times = []
        if (
            self._vlm_model_type == VlmModelType.OPENAI_COMPATIBLE
        ):
...
class VlmPipeline:
    ...
    def get_models_info(self):
        ...
        id = ""
        owned_by = ""
        if self._args.vlm_model_type == VlmModelType.OPENAI_COMPATIBLE:
            from models.openai_compat.openai_compat_model import CompOpenAIModel
    ...
    def populate_argument_parser(parser: ArgumentParser):
        ...
        parser.add_argument(
            ...
            default=VlmModelType.OPENAI_COMPATIBLE,
